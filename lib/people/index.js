...
var INJECT = ['logger', 'presence', 'classes', 'commands', 'switches'];

var PersonManager = require('./manager');
...
function factory(services) {
  ...
  var manager = new PersonManager(presence, switches, commands, logger);
  var personFactory = require('./factory')(manager, logger);
  classes.addClass('person', personFactory);

  return manager;

  // // create presence item for each person configured
  // // bind to the root presence item
  ...
  //   var pp = presence.add('person.'+person.id, {
  //     category:'person',
  //     name:person.name,
  ...
  //   var item = {id: person.id, name: person.name};
  //   Object.defineProperty(item, 'presence', {
  //     enumerable: true,
  //     get: function() { return pp.isPresent; }
  //   });
  //   svc.all[person.id] = item;
  // });
